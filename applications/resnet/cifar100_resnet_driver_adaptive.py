...
def one_batch_size_run(training_generator, num_epochs, params, batch_stats):
	# Training loop
...
# Initial run for initial batch size
params, batch_stats = one_batch_size_run(training_generator,75,params, batch_stats)

gbatches = [64,128,256,512,1024]

for gbatch_size in gbatches:
	# torch.cuda.empty_cache()
	print(80*'#')
	print('Running for gbatch = ',gbatch_size)
	training_generator = data.DataLoader(train_set,
										 batch_size=gbatch_size,
										 shuffle=True,
										 drop_last=True,
										 collate_fn=numpy_collate,
										 num_workers=1,
										 persistent_workers=True,
										 worker_init_fn = np.random.seed(args.run_seed))
	params, batch_stats = one_batch_size_run(training_generator,5, params, batch_stats)
...
logging_dir = 'cifar100_resnet_logging/'
