...
parser.add_argument('-take_one_eventually', '--take_one_eventually', type=float, default=1.0, help="Go up to alpha_k = 1")
...
mushroom_data = np.load('mushroom_train_test_90.npz')

train_images = mushroom_data['Xtrain']
test_images = mushroom_data['Xtest']
train_labels = np.squeeze(mushroom_data['ytrain']).astype(int)
test_labels = np.squeeze(mushroom_data['ytest']).astype(int)
train_labels[np.where(train_labels == 2)] = -1
test_labels[np.where(test_labels==2)] = -1
...
for outer_counter, gbatch_size in enumerate(gbatch_sizes):
	...
	if args.take_one_eventually:
		...
		if args.lr_schedule == 'piecewise':
			...
			print('Using no lr schedule')
			lr_schedule = None
	optimizer.lr_schedule = lr_schedule
	
	for epoch in range(epoch_counter,(num_epochs)*(outer_counter+1)):
		...
		for i_batch in range(n_batches):
			...
			if hbatch_size < gbatch_size:
				hbatch = {'image':hbatch['image'][:hbatch_size],'label':hbatch['label'][:hbatch_size]}
...
logging_dir = 'mushroom_logistic_logging/'
